...
可以通过 `context_as_file`、`current_input_file`、`history_as_file` 或 `forcehistory` 明确开关；模型名含 `-forcehistory` 时总是封装。该后缀可与 `-nothinking` 叠加，且只会在模型 ID 结尾生效。2026-08-30 与 2026-09-05 使用现有上游账号做位置标记实测：GLM-5.3 对单个 48.5 KiB 文本能看到末尾，49 KiB 起稳定出现截断；GLM-5.2 对 256 KiB 可看到最终标记。因此 `glm-5.3` 的历史和工具转写按不超过 48.5 KiB/片上传，每片带 `segment X/Y` 内容头，输入框要求按该编号（而非随机文件名）读取全部分片；160 KiB 内容会按 48.5 KiB 上限自动分片并由编号排序读取。其他模型保留单文件策略，单个生成文件的上传保护上限为 4 MiB。历史与工具定义使用随机短数字文件名（`111.txt` 风格）；同账号同分片内容 10 分钟内复用已上传的 file id。生成文件只在本地临时目录存活到上游上传结束，随后立即删除；任一分片失败时整包回退直传，不会发送半包。
